/**
 * VERSION: 12.0 (Ready to Test Edition)
 * TAGS: #NAVIGATION, #ATTENDANCE, #EXAM, #SURVEY
 * DESCRIPTION: เพิ่มหน้า Welcome Screen ก่อนเริ่มทำข้อสอบ
 */
...
async function openAttendanceForm() {
  ...
  try {
    ...
    if (result.status === 'success') {
      ...
      result.schedule.forEach(slot => {
        ...
        if (logged) {
          container.innerHTML += `<div class="card mb-3 p-3 bg-light border-0 rounded-4 text-center opacity-75 shadow-sm"><div class="fw-bold text-secondary">✔️ ${slot.slot_label} บันทึกสำเร็จ</div><small class="text-muted">เมื่อ: ${logged}</small></div>`;
        } else {
  ...
  } catch (e) { container.innerHTML = 'ผิดพลาดในการเชื่อมต่อ'; }
}

async function submitRealAttendance(dayNo, timeSlot, timeStatus) {
  const swalResult = await Swal.fire({ title: 'หมายเหตุการลงเวลา', input: 'text', inputPlaceholder: 'ระบุหมายเหตุ (ถ้ามี)', showCancelButton: true });
  if (!swalResult.isConfirmed) return; 
  Swal.fire({ title: 'กำลังบันทึก...', didOpen: () => Swal.showLoading() });
  await fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'submitAttendance', payload: { personal_id: localStorage.getItem("tms_personal_id"), day_no: dayNo, time_slot: timeSlot, note: `[${timeStatus}] ${swalResult.value || ""}` } }) });
  Swal.fire('สำเร็จ', 'บันทึกเวลาแล้ว', 'success').then(() => openAttendanceForm());
}

// [COMMENT: #EXAM - เพิ่มหน้า Welcome Screen ตามคำขอพี่บาบู]
...
async function openExamForm() {
  ...
  try {
    ...
    if (result.status === 'success') { 
      ...
      renderExamStartScreen(); // เรียกใช้ฟังก์ชันที่พี่ขอ
    } 
    else if (result.status === 'taken') { contentArea.innerHTML = `<div class="alert alert-success text-center rounded-4 p-5"><h5>${result.message}</h5></div>`; }
    else { contentArea.innerHTML = `<div class="alert alert-warning text-center rounded-4 p-5">${result.message}</div>`; }
  } catch (e) { contentArea.innerHTML = 'ขาดการเชื่อมต่อ'; }
}

// [COMMENT: ฟังก์ชันหน้า Welcome ก่อนเริ่มสอบ]
function renderExamStartScreen() {
  ...
  // เช็คสิทธิ์สอบซ่อม (ถ้ามีลอจิกสอบตกมาก่อน)
  if (globalExamData.isRetake) {
    retakeMessage = `<div class="alert alert-warning mb-3">⚠️ คุณสอบไม่ผ่านในรอบแรก คุณมีสิทธิ์สอบแก้ตัวอีก 1 ครั้ง</div>`;
    btnLabel = 'เริ่มสอบซ่อม';
    ...
    <div class="text-center my-5 p-4 bg-light rounded-4 border shadow-sm">
      ...
      <div class="alert alert-info text-start small mx-auto" style="max-width: 450px;">
        <ul class="mb-0">
          ...
          <li>🚫 ห้ามสลับแท็บหรือสลับหน้าจอ</li>
        </ul>
      </div>
...
function renderQuestions() {
  ...
  globalExamData.questions.forEach((q, i) => {
    html += `<div class="card mb-4 p-4 border-0 shadow-sm rounded-4"><p class="fw-bold fs-5">${i + 1}. ${q.question}</p>`;
...
async function submitRealExam() {
  ...
  Swal.fire({ title: 'กำลังส่งคำตอบ...', didOpen: () => Swal.showLoading() });
...
async function openSurveyForm(type) {
  ...
  area.innerHTML = '<div class="text-center p-5"><div class="spinner-border text-success"></div></div>';
  ...
  try {
    ...
    if (type === 'SPEAKER_SURVEY') { renderSpeakerGrid(); area.innerHTML = '<p class="text-center text-muted p-5">โปรดเลือกวิทยากรด้านบนเพื่อแสดงแบบประเมิน</p>'; }
    else { document.getElementById("speakerSelectionArea").classList.add("d-none"); renderSurveyQuestions(); document.getElementById("btnSubmitSurvey").classList.remove("d-none"); }
  } catch (e) { area.innerHTML = 'ผิดพลาดในการโหลดข้อมูล'; }
}
...
async function submitRealSurvey() {
  ...
  Swal.fire('สำเร็จ', 'บันทึกเรียบร้อย', 'success').then(() => backToDashboard('surveySection'));
}